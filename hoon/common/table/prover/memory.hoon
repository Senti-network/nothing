...
++  test
  ...
  =/  dat  (fink:fock nock)
  =/  [base-table pool]  (get-or-create-table pool 0 |=(* (pad:funcs (build:funcs q.dat))))
  =/  [ext-table pool]  (get-or-create-table pool 1 |=(* (extend:funcs base-table ext-chals q.dat)))
  =/  mid-table  (weld-exts:tlib base-table ext-table)
  =/  [mega-ext-table pool]  (get-or-create-table pool 2 |=(* (mega-extend:funcs mid-table mega-ext-chals q.dat)))
...
:: Memory pool for frequently used data structures
+$  memory-pool
  $:  base-tables=(map @ table-mary)
      ext-tables=(map @ table-mary)
      mega-ext-tables=(map @ table-mary)
      codewords=(map @ mary)
  ==
...
++  get-or-create-table
  |=  [pool=memory-pool key=@ create=gate]
  ^-  [table-mary memory-pool]
  =/  cached  (~(get by base-tables.pool) key)
  ?~  cached
    =/  new  (create)
    [new pool(base-tables (~(put by base-tables.pool) key new))]
  [u.cached pool]

:: Get or create codeword from pool
++  get-or-create-codeword
  |=  [pool=memory-pool key=@ create=gate]
  ^-  [mary memory-pool]
  =/  cached  (~(get by codewords.pool) key)
  ?~  cached
    =/  new  (create)
    [new pool(codewords (~(put by codewords.pool) key new))]
  [u.cached pool]
